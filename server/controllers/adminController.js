...
export const getKnowledgeEntries = async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT ke.*, mc.display_name as model_name 
       FROM knowledge_entries ke
       LEFT JOIN model_configs mc ON ke.model_id = mc.id
...
export const createKnowledgeEntry = async (req, res) => {
  try {
    const { title, type, model_id, content } = req.body;
    let filePath = null;
    let originalFilename = null;

    if (!title || !type || !model_id) {
      return res.status(400).json({ error: 'Title, type, and model are required' });
    }
    ...
    if (type === 'pdf' && req.file) {
      ...
      return res.status(400).json({ error: 'PDF file is required for PDF entries' });
    }

    const result = await pool.query(
      `INSERT INTO knowledge_entries (title, type, content, file_path, original_filename, model_id, created_by) 
       VALUES ($1, $2, $3, $4, $5, $6, $7) 
       RETURNING *`,
      [title, type, content || null, filePath, originalFilename, model_id, req.user.id]
    );
